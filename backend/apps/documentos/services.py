...
def _save_file(archivo) -> str:
    """
    Guarda el archivo en BASE_DIR/media/documentos/<uuid><ext>
    y devuelve la URL relativa: /media/documentos/<uuid><ext>
    """
...
def listar_documentos(id_sac=None):
    """Devuelve todos los documentos. Si se pasa id_sac, filtra por sacramento."""
...
def crear_documento(validated_data: dict, usuario) -> Documentos:
    """
    Guarda el archivo en disco y crea el registro en la BD.
    validated_data proviene de DocumentoUploadSerializer.
    """
...
def cambiar_estado_documento(id_doc: int, estado: str, observacion: str = '', usuario=None) -> Documentos:
...
def eliminar_documento(id_doc: int) -> None:
    ...
    ruta = os.path.join(settings.BASE_DIR, doc.url_doc.lstrip('/'))
